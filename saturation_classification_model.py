...
data = pd.read_csv("D:/PycharmProjects/mP2/saturation_dataset.csv")
...
destination_directory = 'D:/PycharmProjects/mP2/destination_saturation'
    ...
    if ifFace:
        for image_path, class_name in zip(photo_paths_list, classificator):
            ...
            if face is not None:
                ...
                #################################
                ## Sepia
                # img = np.array(face, dtype=np.float64)
                ...
                img1 = cv2.convertScaleAbs(face, beta=50)
                img2 = cv2.convertScaleAbs(img1, beta=-50)
                save_path = os.path.join(destination_path, class_name, os.path.basename(image_path))
